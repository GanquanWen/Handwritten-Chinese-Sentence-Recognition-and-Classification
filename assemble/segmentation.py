...
class CharSeg():
    ...
    def __init__ (self, dir_source, file_name, output_size = (300, 300), min_gray = 200, threshold = 2, min_gap = 3, rows = 1, size_flex = 0.7):
    ...
    def cutImage(self, img, img_gray, vertical_ranges, min_size, output_size, file_name):
        results = []
        for y, vertical_index_pair in enumerate(vertical_ranges):
            horizontal = self.scan_image_horizontal(img_gray[vertical_index_pair[0] : vertical_index_pair[1], : ], self.min_gray)
            ...
            for x, horizontal_index_pair in enumerate(horizontal_ranges):
                ...
                # cv2.imwrite(dir_saved + file_name + "/" + "row" + str(y) + "pos" + str(x) + ".jpg", resized_cutted_char)
    ...
    def run(self):
        ...
        # print(vertical_segs)
        result = self.cutImage(self.image, self.image_gray, vertical_segs, char_height * self.size_flex, self.output_size, self.file_name)
        # print(result)
        # print(len(result))
